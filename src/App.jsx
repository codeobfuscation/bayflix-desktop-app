...
export default function App() {
  ...
  useEffect(() => {
    ...
    window.electron.getAppVersion().then((version) => {
      const lastVersion = localStorage.getItem("bayflix_lastVersion");
      if (lastVersion && lastVersion !== version) {
        clearAppCaches();
      }
      localStorage.setItem("bayflix_lastVersion", version);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("bayflix:library-sort-changed", handler);
    return () =>
    ...
    <ErrorBoundary>
      ...
      <div>
        ...
        {updateBanner && (
          ...
          >
            <span>🎉 Bayflix v{updateBanner.latest} is available!</span>
            <button
