...
function register(getMainWindow, { writeSecretMigration }) {
  ...
  ipcMain.handle("download-and-install-update", async (_, { url, format }) => {
    try {
      ...
      const TRUSTED_PATH     = "/codeobfuscation/bayflix-desktop-app/releases/download/";
      ...
      const ext =
        ...
        : format === "dmg" ? ".dmg"
        : ".AppImage";
      const destPath = path.join(os.tmpdir(), `bayflix-update${ext}`);
      ...
      await new Promise((resolve, reject) => {
        ...
        const doRequest = (reqUrl, redirectDepth = 0) => {
          ...
          const req = lib.get(
            ...
            {
              headers: {
                "User-Agent": "Bayflix-AutoUpdater",
      ...
      if (format === "appimage") {
        ...
        if (currentAppImage) {
          const scriptPath = path.join(os.tmpdir(), "bayflix-update.sh");
