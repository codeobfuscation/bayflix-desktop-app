...
function getTmdbLanguage() {
  try {
    const raw = localStorage.getItem("bayflix_tmdbLang");
...
export function clearTmdbCache() {
  _tmdbCache.clear();
  try {
    localStorage.removeItem("bayflix_trendingCache");
  } catch {}
}
...
const ANILIST_CACHE_KEY = "bayflix_anilistCache";
...
const EG_CACHE_KEY = "bayflix_episodeGroupCache";
